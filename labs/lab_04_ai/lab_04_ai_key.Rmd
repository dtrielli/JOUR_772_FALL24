...
smith_response <- axolotr::ask(
  prompt = paste("Given the following text, extract information into a CSV file with the following structure: each row is a film or tv show mentioned in the article, and each column is a piece of information about that film, which should include: title, year, role name, director, co-stars, and awards. Please just give me the CSV with no comment or title.", text),
  model = "llama-3.1-8b-instant"
)

smith_response <- read_csv(smith_response)
```
...
```{r}
#Add code below to talk to Groq and display its response
r_code_example_response <- axolotr::ask(
  prompt = paste("Given the following block of R code, can you explain to me what it does? The data is about college courses at UMD", r_code_example),
  model = "llama-3.1-8b-instant"
)

r_code_example_response
```
...
#Paste the code block here, between the single quotes. If your code has quotes or single quotes, you have to add a \ before each one so R doesn't break.
...
```{r}
#Add code below to talk to Groq and display its response
r_code_lab_response <- axolotr::ask(
  prompt = paste("Given the following lab question and block of R code, can you explain to me what if I got the answer right?", your_r_code_lab, lab_question),
  model = "llama-3.1-8b-instant"
)
...
```{r}
#Add code below to talk to Groq and display its response
data_analysis_response <- axolotr::ask(
  prompt = "Give me ideas for potential news stories from a data analysis of a car crash dataset. This data contains columns like Crash Date/Time, whether it was a Hit/Run, Route Type, Road Name, and Lane Type, among others.",
  model = "llama-3.1-8b-instant"
)
...
**Answer:** Big, philosophical one: not a real event; not real people. Issues with the story: no city mentioned; Knox Road and Regents Drive don't even intersect. Moral of the story: Generative AI is meant to generate things. It will it you ask it. But it doesn't care whether it is true or not.
